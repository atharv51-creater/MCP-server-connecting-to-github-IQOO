...
export function buildMcpServer() {
  ...
  server.tool(
    "git_status",
    "Get the current git status of the working repo",
  ...
  server.tool(
    "get_diff",
    "Get the current diff (staged or unstaged) so the agent or phone user can review before acting",
    { repoPath: z.string().default("."), staged: z.boolean().default(false) },
  ...
  server.tool(
    "request_commit",
    "Ask for approval to commit the current changes. Sends an actionable notification to the phone.",
    { repoPath: z.string().default("."), message: z.string() },
  ...
  server.tool(
    "request_push",
    "Ask for approval to push committed changes. Sends an actionable notification to the phone.",
    { repoPath: z.string().default(".") },
    async ({ repoPath }) => {
      const action = createPendingAction({
        type: "push",
  ...
  server.tool(
    "request_pull",
    "Ask for approval to pull latest changes. Sends an actionable notification to the phone.",
    { repoPath: z.string().default(".") },
    async ({ repoPath }) => {
      const action = createPendingAction({
        type: "pull",
  ...
  server.tool(
    ...
    "Execute a pending action after the phone approved it (internal — called by the /action HTTP relay, not typically by the agent directly)",
    { actionId: z.string() },
    async ({ actionId }) => {
